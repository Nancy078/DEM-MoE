...
class AnnotatorAwareRegressor(nn.Module):
    ...
    Args:
        ...
        num_labels:     number of discrete rating levels (for label embeddings)
    ...
    def forward(
        ...
        Args:
            ...
            annotator_ids:  (B,)   encoded annotator indices
            annotation_ids: (B,)   discrete rating indices (used for En)
        ...
        # En: leave-one-out average of past label embeddings for this annotator
        sum_i = self.sumEmb[annotator_ids]
        cnt_i = self.count[annotator_ids].unsqueeze(-1).float()
        E_lk  = self.label_embeddings(annotation_ids)
        if self.training:
            En = (sum_i - E_lk) / (cnt_i - 1).clamp(min=1.0)
        else:
            avg_emb     = sum_i / cnt_i.clamp(min=1.0)
            global_mean = self.label_embeddings.weight.mean(dim=0, keepdim=True)
